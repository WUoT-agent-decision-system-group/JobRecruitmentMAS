...
class CandidateModule():
    ...
    def try_add_candidate(self, candidate_to_add: CandidateProfile):
        ...
        if cand is None:
            self.logger.info("Candidate %s does not exist yet. Adding...", candidate_to_add.id)
            return self.__candidate_repository.create(candidate_to_add) is not None
        else:
            self.logger.info("Candidate %s already exists. Updating...", candidate_to_add.id)
            return self.__candidate_repository.update_applied_jobs(candidate_to_add.id,
                                                                   candidate_to_add.applied_jobs)
